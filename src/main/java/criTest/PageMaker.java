...
public class PageMaker {
	private int totalRowCount;  // 전체 row 갯수 (전체 page수 게산위함)
	...
	private boolean prev;  // 이전 PageBlock 으로 이동여부
	private boolean next;  // 다음 PageBlock 으로 이동여부
	...
	public void calcData() {
		...
		// => 계산의 결과로 얻어진 ePageNo가 실제 lastPageNo 보다 크면 수정
		lastPageNo=(int)Math.ceil(totalRowCount/(double)cri.getRowsPerPage());
		// 예) 11/3 -> 3 , 11/(double)3 -> 3.3333
		//    Math.ceil(3) -> 3, Math.ceil(3.3333) -> 4
